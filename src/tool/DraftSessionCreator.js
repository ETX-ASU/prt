...
import {generateDefaultRubric} from "./ToolUtils";
...
function DraftSessionCreator(props) {
  ...
  const rubric = (toolAssignmentData.rubric.criteria.length) ? toolAssignmentData.rubric : generateDefaultRubric();
      ...
      <Fragment key={index}>
        <Card className='rank-card' draggable={true} value={index}
          ...
          <div className='card-content'>
              ...
              <Button className='d-inline-block text-center xbg-dark p-0 rank-visibility-btn' onClick={() => onToggleRankVisibility(index)} >
  ...
  return (
    <Container className='m-0'>
      ...
      <Row className='m-0 p-0'>
        <Col className='p-0'>
          <Container className={`ranks-panel${isDraggingActive ? ' dragging-active' : ''}`}>
